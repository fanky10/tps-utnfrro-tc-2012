...
//        debug("generating cte tiempo "+value);
    ...
    public double getTiempoSubida(DataInput di) {
        ...
        tmin = di.getTau() * Math.log(di.getTau()*ytmin);//-(di.getTau() * (Math.log(-((ytmin-1)/di.getAmplitud()))));
    ...
    protected double getfdet(DataInput di, double time) {
        return (1/di.getTau())*Math.pow(Math.E, (-time/di.getTau()));//from pdf
//        return (1/di.getTau())*di.getAmplitud()*(Math.pow(Math.E, (-time/di.getTau())));//from script
//        return (1/di.getTau())*Math.pow(Math.E, (-time/di.getTau()));//merge script - pdf
    }
